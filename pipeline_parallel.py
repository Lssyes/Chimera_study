...
mp.set_start_method('spawn', force=True)  # 设置多进程的启动方式
...
def start_comm_process(func, kwargs):
    comm_process = Process(target=func, kwargs=kwargs)
...
class PipelineStage:
    ...
    def init_comm_queues(self):
        if not self.is_last_stage:
            for key in self.keys_for_next_stage:
                ...
                self.backward_recv_queues[key] = mp.Queue()
                self.forward_send_queues[key] = mp.Queue()
        if not self.is_first_stage:
            for key in self.keys_from_prev_stage:
                ...
                self.forward_recv_queues[key] = mp.Queue()
                self.backward_send_queues[key] = mp.Queue()
    ...
    @staticmethod
    def recv_comm_process(num_iterations, queue, src_rank, tag, tensor_shape, device):
        for _ in range(num_iterations):
            recv_tensor = torch.zeros(tensor_shape, requires_grad=True)
            if dist.get_backend() == dist.Backend.NCCL:
                recv_tensor = recv_tensor.to(device)
            dist.recv(tensor=recv_tensor, src=src_rank, tag=tag)
            queue.put(recv_tensor.to(device))
    ...
    def send_comm_process(num_iterations, queue, dst_rank, tag):
        for _ in range(num_iterations):
            send_tensor = queue.get()
            if dist.get_backend() != dist.Backend.NCCL:
                send_tensor = send_tensor.cpu()
            dist.send(tensor=send_tensor, dst=dst_rank, tag=tag)
    ...
    def start_comm_threads(self, num_iterations):
        def start_recv_threads(recv_queues, src_rank, tensor_shapes):
            for key, queue in recv_queues.items():
                ...
                #                        tensor_shapes[key],
                #                        device=self.device))
                start_comm_process(self.recv_comm_process,
                                  dict(num_iterations=num_iterations,
                                       queue=queue,
                                       src_rank=src_rank,
                                       tag=self.tag,
        ...
        def start_send_threads(queues, dst_rank):
            for key, queue in queues.items():
                ...
                #                        dst_rank=dst_rank,
                #                        tag=self.tag))
                start_comm_process(self.send_comm_process,
                                  dict(num_iterations=num_iterations,
                                       queue=queue,
                                       dst_rank=dst_rank,
                                       tag=self.tag))
    ...
    def start_interleaved_pipeline_comm_threads(self, num_iterations):
        def start_recv_threads(recv_queues, src_rank, tensor_shapes, tag):
            for key, queue in recv_queues.items():
                start_comm_process(self.recv_comm_thread,
                                  dict(num_iterations=num_iterations,
                                       queue=queue,
                                       src_rank=src_rank,
                                       tag=tag,
        ...
        def start_send_threads(queues, dst_rank, tag):
            for queue in queues.values():
                start_comm_process(self.send_comm_thread,
                                  dict(num_iterations=num_iterations,
                                       queue=queue,
                                       dst_rank=dst_rank,
                                       tag=tag))
