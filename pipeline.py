...
class PipelineStage:
    ...
    #     nvtx.range_push(f"recv_comm_thread_{tag} [{src_rank}->{device}] {num_iterations}x")
    #     for _ in range(num_iterations):
    #         recv_tensor = torch.zeros(tensor_shape, requires_grad=True)
    #         if dist.get_backend() == dist.Backend.NCCL:
    #             recv_tensor = recv_tensor.to(device)
    #         nvtx.range_push(f"recv_comm_thread_{tag} num:<{_}>")
    ...
    #     nvtx.range_push(f"send_comm_thread{tag} [?->{dst_rank}] {num_iterations}x")
    #     for _ in range(num_iterations):
    #         send_tensor = queue.remove()
    #         if dist.get_backend() != dist.Backend.NCCL:
    #             send_tensor = send_tensor.cpu()
    #         nvtx.range_push(f"send_comm_thread{tag} num:<{_}>")
    ...
    def recv_comm_thread(num_iterations, queue, src_rank, tag, tensor_shape, device):
        ...
        for _ in range(num_iterations):
            recv_tensor = torch.zeros(tensor_shape, requires_grad=True)
            recv_tensor_fake = torch.zeros(1, dtype=torch.uint8)
            if dist.get_backend() == dist.Backend.NCCL:
                recv_tensor = recv_tensor.to(device)
                recv_tensor_fake = recv_tensor_fake.to(device)
            nvtx.range_push(f"recv_comm_thread_{tag} num:<{_}>")
            dist.recv(tensor=recv_tensor_fake, src=src_rank, tag=tag)
            queue.add(recv_tensor.to(device))
            nvtx.range_pop()
        nvtx.range_pop
    ...
    def send_comm_thread(num_iterations, queue, dst_rank, tag):
        ...
        for _ in range(num_iterations):
            send_tensor = queue.remove()
            send_tensor = torch.ones(1, dtype=torch.uint8).to(send_tensor.device)
    ...
    # @staticmethod
    # def recv_comm_thread_async(num_iterations, queue, src_rank, tag, tensor_shape, device):
    #     requests = []
    #     for _ in range(num_iterations):
    #         recv_tensor = torch.zeros(tensor_shape, requires_grad=True)
    #         if dist.get_backend() == dist.Backend.NCCL:
    #             recv_tensor = recv_tensor.to(device)
    #         req = dist.irecv(tensor=recv_tensor, src=src_rank, tag=tag)
    #         requests.append((req, recv_tensor))
    ...
    # @staticmethod
    # def send_comm_thread_async(num_iterations, queue, dst_rank, tag):
    #     requests = []
    #     for _ in range(num_iterations):
    #         send_tensor = queue.remove()
    #         if dist.get_backend() != dist.Backend.NCCL:
    ...
    #         req = dist.isend(tensor=send_tensor, dst=dst_rank, tag=tag)
    #         requests.append(req)
    ...
    def call_forward(self, input_source: OrderedDict[str, Tensor]):
        ...
        # [Forward] print the shape of inputs and outputs
        # print(f"\033[0;31;40m[Forward]\033[0m")
        
        # for key in inputs:
        #     print(f"\033[0;31;40m[inputs-key] {key} -> {inputs[key].shape} -> {inputs[key].device}\033[0m")
        # for key in outputs:
        #     print(f"\033[0;31;40m[outputs-key] {key} -> {outputs[key].shape} -> {outputs[key].device}\033[0m")
    ...
    def nb_sync_grad(self):
        ...
        # print the device of grads[-1]
        print(f"\033[0;31;40m[g.device:] {grads[-1].device}\033[0m")
        
        self.grads.append(grads)
        
        packed_tensor = parameters_to_vector(self.grads[-1])
        # print the shape of packed_tensor (vector)
        # print(f"\033[0;31;40m[packed_tensor-shape] vector.shape -> {packed_tensor.shape}\033[0m")
        
        self.packed_grads.append(packed_tensor)
        
        self.handles.append(dist.all_reduce(self.packed_grads[-1], 
                                            group=self.grad_sync_group, async_op=True))
        # print("\033[0;31;40m[nb_sync_grad-mark] 4\033[0m")
        

        nvtx.range_pop()
    
    
    def nb_sync_grad_very_nb(self):
        nvtx.range_push('nb_sync_grad' + self.nvtx_tag)
        ...
        one_byte_buf = torch.zeros(1, dtype=torch.uint8).to(self.device)
        
        self.handles.append(dist.all_reduce(one_byte_buf, 
                                            group=self.grad_sync_group, async_op=True))
        # print("\033[0;31;40m[nb_sync_grad-mark] 4\033[0m")
    ...
    def wait_all(self):
        ...
        # print(f"\033[0;31;40m[wait_all handles number] {len(self.handles)}\033[0m")
    ...
    def call_pipeline(self,
                      ...
                      iteration: int = None,
                      no_sync_grad=False):
        nvtx.range_push(f'call_pipeline_RANK{self.rank}')
    ...
    def _call_chimera_pipeline(self,
        ...
        """
        print("\033[0;36;40m[[[_call_chimera_pipeline]]]\033[0m")
        ...
        def wait_all():
            # print(f"\033[0;36;40m[[[no_sync_grad - value: {no_sync_grad} ]]]\033[0m")
            if not no_sync_grad:
        ...
        def sync_grad(index, down_or_up):
            ...
            if not no_sync_grad:
                ...
                call('nb_sync_grad_very_nb', index, down_or_up)
        
        pipeline = AutoGeneratePipelineRank(self.num_stages, 2, 
                                            num_micro_batches*2)
