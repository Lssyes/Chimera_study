...
# from pipeline_parallel import PipelineStage, PIPELINE_1F1B, PIPELINE_GPIPE, PIPELINE_CHIMERA, PIPELINE_INTER
...
def main():
    ...
    for epoch in range(num_epochs):
        dist.barrier()
        print(f"[[pytorch sync]] {epoch} epoch")
        if num_replicas > 1:
            # deterministically shuffle based on epoch
            train_loader.sampler.set_epoch(epoch)

        steps_for_this_epoch = min(num_steps - total_steps,
                                   max_steps_per_epoch)
        print(f"[[Chimera start]] {epoch} epoch")
        train_one_epoch(epoch, 
                        total_steps, 
                        steps_for_this_epoch)
        
        total_steps += steps_for_this_epoch


    if is_master:
        finish_str = "Master Finished!!!!"
        print(f"\033[0;31;40m{finish_str}\033[0m")
    print(f"\033[0;31;40m[Finished!!!!{rank}]\033[0m")
...
def train_one_epoch(epoch, step, num_steps_for_this_epoch):
    ...
    if interleaved_pipelines:
        ...
        for index, s in enumerate(stages):
            s.start_comm_threads(num_p2p_comm)  # 这里已经改成 torch mp 多进程了
            s.stage_module.train()
            
        print(f"shiiitttt epoch-{epoch} step-{step} num_steps_for_this_epoch-{num_steps_for_this_epoch} num_p2p_comm-{num_p2p_comm}")
    ...
    with save_cxt as cxt:
        ...
        with save_cxt_up_pipe as cxt_up_pipe:
            ...
            for i in range(num_steps_for_this_epoch):
                print(f"\033[0;31;40m[epoch-{epoch} step-{step} start]\033[0m")

                for optimizer in optimizers:
                    optimizer.zero_grad()
                dist.barrier()
                print(f"[[pass of the dist.barrier()]]")
...
if __name__ == "__main__":
    ...
    print(f"local_rank {local_rank}, local_size {local_size}, rank-{rank}, world_size-{world_size}")
    ...
    for _rank in range(world_size):
        if interleaved_pipelines:
            ...
            # Chimera的stage
            pipeline = AutoGeneratePipelineRank(num_stages, num_pipelines,
    ...
    print(f"[[stage_to_ranks]] -> {stage_to_ranks}")
    ...
    print(f"[total_num_samples_per_step: {total_num_samples_per_step}]]]]]]]]]]")
    ...
    if num_steps is None:
        ...
        print(f"total_num_samples: {total_num_samples} num_epochs: {num_epochs}")
